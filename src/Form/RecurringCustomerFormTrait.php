...
/**
 * Provides a form for selecting the recurring's customer (uid and mail fields).
 *
 * Used when adding a new recurring or reassigning an existing one.
 */
trait RecurringCustomerFormTrait {
   ...
   *   The current recurring, if known.
   *
   * @return array
   *   The parent form with the customer form elements added.
   */
  public function buildCustomerForm(array $form, FormStateInterface $form_state, RecurringInterface $recurring = NULL) {
    ...
    if ($selected_customer_type == 'existing') {
      ...
      // The password_confirm element needs to be wrapped in recurring for
      // #states to work properly. See https://www.drupal.org/node/1427838.
